...
export const WORK_ID = "44444444-4444-4444-4444-444444444444";
...
export const TOMORROW = calendarDate(1);
 ...
 */
export async function mockTeachingApis(page: Page) {
    ...
    checks: [
      {
        ...
        passed: true,
        explanation:
          "Language fits Grade 5. QualityExplanationMustWrapAcrossTheCardWithoutOverlappingANeighbourAndRemainReadableOnNarrowViewports",
      },
    ],
  };
  ...
  for (let index = 0; index < CONTENT_IDS.length; index += 1) {
    ...
    await page.route(
      `**/api/v1/contents/${contentId}/versions/${versionId}/actions/request-changes`,
      async (route) => {
        const headers = route.request().headers();
        expect(headers["if-match"]).toBe('"r2"');
        ...
        artifacts = artifacts.map((item) =>
          item.content_id === contentId
            ? { ...item, stewardship_state: "CHANGES_REQUESTED", aggregate_revision: 3 }
            : item,
        );
        await route.fulfill({
          status: 200,
    ...
    await page.route(
      `**/api/v1/contents/${contentId}/versions/${versionId}/actions/reject`,
      async (route) => {
        const headers = route.request().headers();
        expect(headers["if-match"]).toBe('"r2"');
        artifacts = artifacts.map((item) =>
          item.content_id === contentId
            ? { ...item, stewardship_state: "REJECTED", aggregate_revision: 3 }
            : item,
        );
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({
            review_decision_id: "cccccccc-cccc-cccc-cccc-cccccccccccc",
            content_id: contentId,
            version_id: versionId,
            decision: "REJECTED",
            reason_code: null,
