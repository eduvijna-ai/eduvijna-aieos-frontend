"""Bootstrap disposable PostgreSQL 18 for AIEOS360-S04-I03-E2E.

Reuses backend tests/conftest.py identity, migration, and runtime-grant patterns.
NON_PRODUCTION only. Requires AIEOS_BACKEND_ROOT at I03 backend pin.
...
def main() -> int:
    ...
    port = os.environ.get("AIEOS_TEST_PG_PORT", "55438")
    report_path = Path(
        os.environ.get(
    ...
    if external:
        ...
        conftest.CONTAINER_NAME = os.environ.get(
            "AIEOS360_S04_I03_E2E_PG_CONTAINER", "aieos-aieos360-s04-i03-e2e-pg"
        )
