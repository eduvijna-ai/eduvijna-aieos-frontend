"""Serve the NON_PRODUCTION Student OS API for AIEOS360-S04-I03-E2E.
...
def main() -> int:
    ...
    port = int(os.environ.get("AIEOS360_S04_I03_E2E_STUDENT_BACKEND_PORT", "8011"))
    host = os.environ.get(
        "AIEOS360_S04_I03_E2E_STUDENT_BACKEND_HOST", "127.0.0.1"
    )

    engine = create_engine(runtime_url)
    membership = DevelopmentCoherentSchoolContextProvider()

    # Intentionally omit school_context_class_reader from the Student surface.
    app = create_app(
        ...
        cursor_signing_key=b"aieos360-s04-i03-e2e-student-cursor",
