"""Bootstrap disposable PostgreSQL 18 for AIEOS360-S02-I04-E2E.

Reuses backend tests/conftest.py identity, migration, and runtime-grant patterns.
NON_PRODUCTION only. Requires AIEOS_BACKEND_ROOT at I04 backend pin.
...
def main() -> int:
    ...
    port = os.environ.get("AIEOS_TEST_PG_PORT", "55436")
    report_path = Path(
        os.environ.get(
    ...
    if external:
        ...
        conftest.CONTAINER_NAME = os.environ.get(
            "AIEOS360_S02_I04_E2E_PG_CONTAINER", "aieos-aieos360-s02-i04-e2e-pg"
        )
