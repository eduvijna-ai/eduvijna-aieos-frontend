"""Seed I04 Principal OS real-stack precondition (shared PostgreSQL truth).
...
NON_PRODUCTION only.
Backend pin: e2bfce86afece6772eaf7c2f1eb18e2dd2240f1b
...
SCENARIO_ID = "aieos360-s02-i04-e2e-principal-os-real-stack"
BACKEND_PIN_SHA = "e2bfce86afece6772eaf7c2f1eb18e2dd2240f1b"
EXPECTED_MIGRATION_HEAD = "a360s010004"
OPENAPI_AUTHORITY_SHA = (
    "BE60CC2A4612F77AB333088D264B9501B9AB842995AEC1539DA89EA0E8462B47"
)
DEV_PRINCIPAL_BEARER_TOKEN = "aieos360-s02-i04-e2e-principal"
DEV_TEACHER_BEARER_TOKEN = "aieos360-s02-i04-e2e-teacher"
...
def main() -> int:
    ...
    bootstrap_url = os.environ.get("AIEOS360_S02_I04_E2E_BOOTSTRAP_DATABASE_URL")
    db_report_path = Path(
        os.environ.get(
            "AIEOS360_S02_I04_E2E_DB_REPORT",
            Path(__file__).resolve().parents[2] / "tmp" / "aieos360-s02-i04-e2e-db.json",
        )
    )
    ...
    if not runtime_url:
        if not db_report:
            raise SystemExit(
                "AIEOS360_S02_I04_E2E_RUNTIME_DATABASE_URL or "
                "AIEOS360_S02_I04_E2E_DB_REPORT required"
            )
    ...
    if not bootstrap_url:
        if db_report and db_report.get("bootstrap_database_url"):
            ...
            raise SystemExit(
                "AIEOS360_S02_I04_E2E_BOOTSTRAP_DATABASE_URL or "
                "bootstrap_database_url in AIEOS360_S02_I04_E2E_DB_REPORT required"
            )

    fixture_path = Path(
        os.environ.get(
            "AIEOS360_S02_I04_E2E_FIXTURE_PATH",
            Path(__file__).resolve().parents[2]
            / "tmp"
            / "aieos360-s02-i04-e2e-fixture.json",
        )
    )
    ...
    scenario_date = date.fromisoformat(
        os.environ.get("AIEOS360_S02_I04_E2E_SCENARIO_DATE", "2026-09-16")
    )
        ...
        if work_id is None:
            created = client.post(
                ...
                json={
                    ...
                    "subject": "Mathematics",
                    "topic": "Deterministic I04 evidence",
                },
                headers=_headers(
                    str(tenant_id),
    ...
    with bootstrap_engine.connect() as conn:
        ...
        if principal_row.status != "ACTIVE":
            raise RuntimeError(
                f"expected Principal status ACTIVE; got {principal_row.status}"
            )
        if principal_row.principal_kind != "HUMAN":
            raise RuntimeError(
                f"expected Principal kind HUMAN; got {principal_row.principal_kind}"
            )
