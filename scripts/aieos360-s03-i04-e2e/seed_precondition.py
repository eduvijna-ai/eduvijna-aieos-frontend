"""Seed I04 Parent OS real-stack precondition (shared PostgreSQL truth).

Creates:
  - HUMAN teacher principal
  - Student A and Student B principals (development learners)
  - Canonical Parent OS HUMAN adult principal (ACTIVE, HUMAN)
...
Does NOT modify canonical Backend `_DEFAULT_ACCESS`.
NON_PRODUCTION only.
Backend pin: 138f37bfa7a44c33b206c6b78118154bbb9bc8eb
...
SCENARIO_ID = "aieos360-s03-i04-e2e-parent-os-real-stack"
BACKEND_PIN_SHA = "138f37bfa7a44c33b206c6b78118154bbb9bc8eb"
EXPECTED_MIGRATION_HEAD = "a360s010004"
OPENAPI_AUTHORITY_SHA = (
    "4042FB2725DA70A02A70EE09563B7698AE2E5DA82927614CAF1B5F7E6AA7C1D0"
)
ARCHITECTURE_PIN_SHA = "67f4020b78cacdf1716e18d83f4410cb16fdcb4d"
FRONTEND_BASE_SHA = "04c2b1850732df1b8fe5de55f018edf30365181d"
DEV_PARENT_BEARER_TOKEN = "aieos360-s03-i04-e2e-parent"
DEV_TEACHER_BEARER_TOKEN = "aieos360-s03-i04-e2e-teacher"
...
def main() -> int:
    ...
    from aieos.development.parent_learner_access import PARENT_OS_HUMAN_ADULT_A_ID
    ...
    from deterministic_worksheet import (
        ...
        Q_UNANSWERED_ID,
        WORKSHEET_TITLE,
        deterministic_i04_worksheet_model,
    )
    ...
    bootstrap_url = os.environ.get("AIEOS360_S03_I04_E2E_BOOTSTRAP_DATABASE_URL")
    db_report_path = Path(
        os.environ.get(
            "AIEOS360_S03_I04_E2E_DB_REPORT",
            Path(__file__).resolve().parents[2] / "tmp" / "aieos360-s03-i04-e2e-db.json",
        )
    )
    ...
    if not runtime_url:
        if not db_report:
            raise SystemExit(
                "AIEOS360_S03_I04_E2E_RUNTIME_DATABASE_URL or "
                "AIEOS360_S03_I04_E2E_DB_REPORT required"
            )
    ...
    if not bootstrap_url:
        if db_report and db_report.get("bootstrap_database_url"):
            ...
            raise SystemExit(
                "AIEOS360_S03_I04_E2E_BOOTSTRAP_DATABASE_URL or "
                "bootstrap_database_url in AIEOS360_S03_I04_E2E_DB_REPORT required"
            )

    fixture_path = Path(
        os.environ.get(
            "AIEOS360_S03_I04_E2E_FIXTURE_PATH",
            Path(__file__).resolve().parents[2]
            / "tmp"
            / "aieos360-s03-i04-e2e-fixture.json",
        )
    )
    ...
    gateway = FakeStructuredModelGateway(
        result_factory=lambda _request: deterministic_i04_worksheet_model(),
        provider_id="fake",
        model_id="fake-model",
    )
    ...
    scenario_date = date.fromisoformat(
        os.environ.get("AIEOS360_S03_I04_E2E_SCENARIO_DATE", "2026-09-17")
    )
        ...
        if work_id is None:
            created = client.post(
                ...
                json={
                    ...
                    "subject": "Mathematics",
                    "topic": "Deterministic I04 Parent evidence",
                },
                headers=_headers(
                    str(tenant_id),
    ...
    with bootstrap_engine.connect() as conn:
        ...
        if parent_row.status != "ACTIVE":
            raise RuntimeError(
                f"expected Parent status ACTIVE; got {parent_row.status}"
            )
        if parent_row.principal_kind != "HUMAN":
            raise RuntimeError(
                f"expected Parent kind HUMAN; got {parent_row.principal_kind}"
            )
        student_b_row = conn.execute(
            text(
                """
                SELECT status, principal_kind
                FROM security.principals
        ...
        if student_b_row.status != "ACTIVE":
            raise RuntimeError(
                f"expected Student B status ACTIVE; got {student_b_row.status}"
            )
        if student_b_row.principal_kind != "HUMAN":
            raise RuntimeError(
                f"expected Student B kind HUMAN; got {student_b_row.principal_kind}"
            )
