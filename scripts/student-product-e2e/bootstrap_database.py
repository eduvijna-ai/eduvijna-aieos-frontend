"""Bootstrap disposable PostgreSQL 18 for AIEOS360-S01-I04R1 Student product E2E.

Reuses backend tests/conftest.py identity, migration, and runtime-grant patterns.
NON_PRODUCTION only. Requires AIEOS_BACKEND_ROOT at merged I03 SHA.
Governed migration head: a360s010002.
"""
...
def main() -> int:
    ...
    port = os.environ.get("AIEOS_TEST_PG_PORT", "55434")
    report_path = Path(
        os.environ.get(
    ...
    if external:
        ...
        conftest.CONTAINER_NAME = os.environ.get(
            "STUDENT_PRODUCT_E2E_PG_CONTAINER", "aieos-student-product-e2e-pg"
        )
